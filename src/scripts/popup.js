    ...
    browser.storage.local.get(['server', 'bip39', 'network'])
      .then((result) => {
        ...
        const updateDomain = (value) => {
          document.getElementById('webowner').innerHTML = `${domain} has set a tipping address:<br><b>${value}</b>`;
        };
        ...
        if (server !== '' && server !== undefined) {
          browser.runtime.sendMessage({ msg: 'start', server: `${server}` })
            ...
            Removed until there's a workaround for the extra permissions required
            in the manifest.
